...
#define left(i)   ((i) << 1ull)
#define right(i)  (((i) << 1ull) + 1ull)
#define parent(i) ((i) >> 1ull)


pqueue_t *
pqueue_init(u_int64_t n,
...
static void
bubble_up(pqueue_t *q, u_int64_t i)
{
    u_int64_t parent_node;
...
static u_int64_t
maxchild(pqueue_t *q, u_int64_t i)
{
    u_int64_t child_node = left(i);
...
static void
percolate_down(pqueue_t *q, u_int64_t i)
{
    u_int64_t child_node;
    void *moving_node = q->d[i];
    pqueue_pri_t moving_pri = q->getpri(moving_node);
...
static int
subtree_is_valid(pqueue_t *q, u_int64_t pos)
{
    if (left(pos) < q->size) {
        /* has a left child */
